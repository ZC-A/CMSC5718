...
def calculate_option_price(paths, steps):
    ...
    for _ in range(paths):
        ...
        for _ in range(steps):
            ...
            deltaS1 = S1 * np.exp(r * dt + np.sqrt(dt) * z1 * alpha1)
            deltaS2 = S2 * np.exp(r * dt + np.sqrt(dt) * z2 * alpha2)
    ...
    fair_price = average_payoff * np.exp(-r * T)
...
price_10000, time_10000 = calculate_option_price(paths, steps)
...
price_300000, time_300000 = calculate_option_price(paths, steps)
